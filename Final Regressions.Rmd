...
# CrossTable(stack_total$age2, stack_total$year)
...
#assign income midpoints to income_mid in $10000's
data_clean <- data_clean %>%
...
#assign family income midpoints to faminc_mid in $1000's
data_clean <- data_clean %>%
...
data_clean$era <- factor(data_clean$era, labels = c("1", "2"))
...
data_clean2 <- data_clean %>%
  mutate(insample = ifelse( is.na(dr5day) |  is.na(irpinc3) | is.na(treated) |
                              is.na(alctry) | is.na(everdrank) | is.na(evermj)| is.na(evercoc), 0,
                            1))
data_clean <- data_clean2 %>%
  filter(insample == 1)
```
\pagebreak
# Comparing regressions 5/8/18
## Probability: Binge [0][1+] FAMILY Treatedyn
```{r, echo=FALSE}
...
```
\pagebreak
## Negative Binomial: Binge 0-30 w/era FAMILY INCOME Treatedyn
```{r, echo=FALSE}
#negative binomial
...
\pagebreak
# Comparing ERA 5/9/18
## Probability: Binge [0][1+] FAMILY Treatedyn ERA_1
```{r, echo=FALSE}
lm_prob_fam_yn1 <- lm(binge2cat ~ faminc_mid + treatedyn + alctry + evermj + evercoc + 
                     edu4cat + age + sex + marital + dependents + faminc_mid*treatedyn + 
                     edu4cat*treatedyn, data = era_1, na.action = na.omit)
summary(lm_prob_fam_yn1)
```
\pagebreak\pagebreak
# Comparing ERA 5/9/18
## Probability: Binge [0][1+] FAMILY Treatedyn  ERA_2
```{r, echo=FALSE}
lm_prob_fam_yn2 <- lm(binge2cat ~ faminc_mid + treatedyn + alctry + evermj + evercoc + 
                     edu4cat + age + sex + marital + dependents + faminc_mid*treatedyn + 
                     edu4cat*treatedyn, data = era_2, na.action = na.omit)
summary(lm_prob_fam_yn2)
```

\pagebreak
# Era interaction probability
```{r, echo=FALSE}
#Final Models! with ERA interaction
lm_prob_fam_yn_era <- lm(binge2cat ~ faminc_mid + treatedyn + alctry + evermj + evercoc + 
                       edu4cat + age + sex + marital + dependents + era + faminc_mid*treatedyn*era + 
                       edu4cat*treatedyn*era, data = data_clean, na.action = na.omit)
summary(lm_prob_fam_yn_era)
```

\pagebreak
#Era interaction negative binomial
```{r, echo= FALSE}
#negative binomial
nb_famincyn_era <- glm.nb(dr5day ~ faminc_mid + treatedyn + alctry + evermj + evercoc +
                        edu4cat + age + sex + marital + dependents + era + faminc_mid*treatedyn*era +
                        edu4cat*treatedyn*era, data = data_clean, na.action = na.omit)
summary(nb_famincyn_era)
```
